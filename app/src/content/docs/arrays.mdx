...
<Tabs items={['Named Import', 'Namespace Import']}>
  ...
  <Tab value="Namespace Import">
    ...
    type Result = Arrays.ArrayToUnion<["foo", "bar", "foobar"]>
    ```
  </Tab>
</Tabs>

### `ArrayToUnion<Array extends readonly unknown[]>` [#array-to-union]
...
- Produces a union of all values in the array, excluding empty arrays (`[]`).
- Changing the array changes the resulting union.

- Use with `as const` arrays for maximum type safety.
- Combine with other utility types to restrict function parameters or object keys.
...
### `Size<Array extends unknown[]>` [#size]
...
- Returns a numeric literal type representing the length.
- Changing the array changes the result.
...
### `Filter<Array extends unknown[], Predicate, Includes extends boolean = true>` [#filter]
...
- If `Includes` is `true`, only elements assignable to `Predicate` are included.
- If `Includes` is `false`, elements assignable to `Predicate` are excluded.
...
### `Reverse<Array extends unknown[]>` [#reverse]
...
### `IndexOf<Array extends unknown[], Match>` [#index-of]
...
### `LastIndexOf<Array extends unknown[], Match>` [#last-index-of]
...
### `ConstructTuple<Length extends number, Value extends unknown = unknown>` [#constructtuple]
...
### `HasDuplicates<Array extends unknown[]>` [#has-duplicates]
...
### `AllEquals<Array extends unknown[], Comparator>` [#all-equals]
...
### `Chunk<Array extends unknown[], Length extends number>` [#chunk]
...
### `Zip<Array1 extends unknown[], Array2 extends unknown[]>` [#zip]
...
### `CompareArrayLength<T extends any[], U extends any[]>` [#compare-array-length]
...
### `Uniques<Array extends unknown[]>` [#uniques]
...
### `Take<N extends number, Array extends unknown[]>` [#take]
...
### `FilterNonNullish<Array extends unknown[]>` [#filter-non-nullish]
