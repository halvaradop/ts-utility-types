...
describe("Readonly", () => {
	test("DeepReadonly for objects", () => {
		expectTypeOf<DeepReadonly<{ foo: string; bar: number }>>().toEqualTypeOf<{
			readonly foo: string;
			readonly bar: number;
		}>();
		expectTypeOf<DeepReadonly<{ foo: string; bar: { foo: number } }>>().toEqualTypeOf<{
			readonly foo: string;
			readonly bar: { readonly foo: number };
		}>();
		expectTypeOf<DeepReadonly<{ foo: { bar: string }; bar: { foo: number } }>>().toEqualTypeOf<{
...
describe("FlattenProperties", () => {
	test("Extract property from object", () => {
		expectTypeOf<FlattenProperties<{ name: string; address: { street: string } }, "address">>().toEqualTypeOf<{
			name: string;
			street: string;
		}>();
		expectTypeOf<
			FlattenProperties<{ name: string; address: { street: string; avenue: string } }, "address">
...
describe("UnionMerge", () => {
	test("Merge the types of the properties of two objects.", () => {
		...
		expectTypeOf<UnionMerge<{ foo: string | number }, { foo: boolean }>>().toEqualTypeOf<{
...
describe("DeepMutable", () => {
	test("Converts all properties to non readonly of an object type", () => {
		...
		expectTypeOf<DeepMutable<DeepReadonly<{ foo: { bar: { foobar: number } } }>>>().toEqualTypeOf<{
			foo: { bar: { foobar: number } };
		}>();
		expectTypeOf<DeepMutable<DeepReadonly<{ foo: [{ bar: string; foobar: number }] }>>>().toEqualTypeOf<{
...
describe("MergeAll", () => {
	test("Merge the properties of a tuple of objects", () => {
		...
		expectTypeOf<MergeAll<[{ foo: string }, { bar: { foobar: number } }]>>().toEqualTypeOf<{
			foo: string;
			bar: { foobar: number };
		}>();
		type Expect3 = {
		...
		expectTypeOf<
			MergeAll<[{ foo: string }, { bar: string }, { bar: number; foo: boolean; foobar: string }]>
...
describe("AddPropertyToObject", () => {
	test("Append a new property of an exist object type", () => {
		...
		expectTypeOf<AddPropertyToObject<{ foo: string }, "bar", { foobar: number; barfoo: boolean }>>().toEqualTypeOf<{
			foo: string;
			bar: { foobar: number; barfoo: boolean };
		}>();
		expectTypeOf<AddPropertyToObject<{ foo: string }, "bar", [1, 2, 3]>>().toEqualTypeOf<{ foo: string; bar: [1, 2, 3] }>();
		expectTypeOf<AddPropertyToObject<{ foo: string }, "bar", string | boolean | number>>().toEqualTypeOf<{
			foo: string;
...
describe("Intersection", () => {
	test("Intersection of properties between two objects", () => {
		...
		expectTypeOf<
			Intersection<{ foo: string; bar: { bar: number } }, { barfoo: { bar: number }; foo: bigint }>
...
describe("Tuple methods", () => {
	...
	describe("FilterOut", () => {
		test("Removes the elements present in the predicate", () => {
			...
			expectTypeOf<FilterOut<[{ foo: string }, { bar: number }, { foobar: boolean }], { bar: number }>>().toEqualTypeOf<
				[{ foo: string }, { foobar: boolean }]
			>;
			expectTypeOf<FilterOut<["foo", "bar", "foobar", 1, 2, { foo: string }], { foo: string }>>().toEqualTypeOf<
				["foo", "bar", "foobar", 1, 2]
			>;
		});
	});
	...
	describe("Reverse", () => {
		test("Reverse the elements of a tuple type", () => {
			...
			expectTypeOf<Reverse<[1, "foo", 2, "bar", { foo: number }, () => void]>>().toEqualTypeOf<
...
describe("Omit Properties", () => {
	...
	describe("OmitByType", () => {
		test("Omit the properties based on the type", () => {
			...
			expectTypeOf<OmitByType<{ foo: string; bar: number; foobar: boolean }, string | boolean>>().toEqualTypeOf<{
				bar: number;
			}>();
			expectTypeOf<
				OmitByType<
...
describe("ObjectEntries", () => {
	test("Returns the entries from an object", () => {
		...
		expectTypeOf<ObjectEntries<{ foo?: undefined; bar: undefined | string }>>().toEqualTypeOf<
...
describe("Pick Utilities", () => {
	...
	describe("PickByType", () => {
		test("Pick by type", () => {
			...
			expectTypeOf<PickByType<{ foo: string; bar: number; foofoo: string }, string>>().toEqualTypeOf<{
				foo: string;
				foofoo: string;
			}>();
